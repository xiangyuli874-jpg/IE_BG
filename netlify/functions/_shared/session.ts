...
import { DEFAULT_PARAMETERS, findLine, findModel } from "../../../src/data/presets";
import type { SampleValue, WorktimeGroup, WorktimeParameters, WorktimeProcess, WorktimeSession } from "../../../src/types";
...
function normalizeGroups(groups: unknown): WorktimeGroup[] {
  ...
  const normalized = groups.slice(0, MAX_GROUPS).map((group, groupIndex) => {
    ...
    return {
      ...
      order: groupIndex + 1,
      processes: processes.slice(0, MAX_PROCESSES).map((process, processIndex) => normalizeProcess(process as Partial<WorktimeProcess>, processIndex))
    };
  });
...
export function createSession(lineId: string, modelId = DEFAULT_TEMPLATE_MODEL_ID, modelName = ""): WorktimeSession {
  ...
  const groups = structuredClone(model.groups).map((group, groupIndex) => ({
    ...
    processes: group.processes.map((process, processIndex) => ({
      ...
      name: "",
      samples: [null, null, null, null, null] as [SampleValue, SampleValue, SampleValue, SampleValue, SampleValue],
